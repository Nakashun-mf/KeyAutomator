...
## システム要件（Partner Center「アプリのプロパティ」）

Store 登録情報の **[システム要件]** に出る項目です。  
**最小**に入れた機能が無い PC では、ダウンロード前に警告が出て評価・レビュー不可になります。不要な機能を最小に入れないこと。

### ハードウェア機能（チェックボックス）

| 機能 | 最小要件 | 推奨要件 | 理由 |
|---|---|---|---|
| **キーボード** | ✅ オン | （オフで可） | キー入力マクロが本アプリの主機能 |
| **マウス** | ✅ オン | （オフで可） | GUI 操作＋マウスクリック手順のため |
| タッチ | オフ | オフ | 必須ではない（タッチでも動くが不要） |
| カメラ / マイク / NFC / Bluetooth / GPS 等 | オフ | オフ | 未使用。オンにすると誤警告の原因 |
...
タッチ専用端末向けに制限する必要はないので、タッチを最小にしないでください。

### メモリなど（テキスト欄がある場合）

空欄でも問題ありません。入れるなら控えめに:

| 欄 | 推奨記入 |
|---|---|
| 最小メモリ | `2 GB`（または空欄） |
| 推奨メモリ | `4 GB`（または空欄） |
| 最小のプロセッサ | 空欄、または `1 GHz 以上` |
| 推奨のプロセッサ | 空欄、または `x64` |
| グラフィックス | 空欄（専用 GPU 不要） |
| 空きディスク | 空欄、または `200 MB` |

専用 GPU・カメラ・コントローラー・MR ヘッドセットなどは **一切指定しない**。

### ストアに併記する説明文（任意・プロパティ外の説明欄用）
