# キー入力自動化アプリケーション (C# / WinUI 3) 基本設計仕様書

## 1. アプリケーション概要

本アプリケーションは、事前登録された一連のキー入力シーケンス（テキスト送信、特殊キー押下、ショートカットキー操作、ステップ間ウェイト、確認ダイアログ）を、指定秒数待機後に「アクティブウィンドウ」へ自動入力するWindowsデスクトップツールである。
...
- **表示言語**: 日本語 / English。既定は Windows の表示言語。`settings.json` の `ui_language`（空 / `ja-JP` / `en-US`）で上書きできる。
...
[起動コマンド]
  ...
  └── 引数あり (例: -1)         => CLI実行モード（管理画面は出さない。dialog 手順はメッセージ表示）
```
...
- 動作手順:
  ...
  4. 待機終了時のアクティブウィンドウに対して、登録されたアクションリストを順番に実行。
     - 各アクションのあいだに `settings.json` の `action_delay_sec`（既定 0.2 秒）を自動挿入する。
  5. 全アクション完了後、即座にアプリケーションを正常終了（Exit Code: 0）。
- エラーハンドリング:
  - 該当するID/名称が存在しない場合、ログ（オプション）を残して即座に終了（Exit Code: 1）。UIダイアログなどのポップアップは出力しない。
  - ただしアクション種別 `dialog` は意図的な確認ポイントのため、CLI 実行時もメッセージボックスを表示する。
- GUI の「起動パスをコピー」は、バッチに貼る起動コマンドをクリップボードへ入れる。
  マクロ選択中はパスに `-alias`（無ければ `-id`）を付ける。未選択ならパスのみ。
  パッケージ実行時のパスは AppExecutionAlias（`%LocalAppData%\Microsoft\WindowsApps\KeyAutomator.exe`）。設定フォルダ（config.json）とは別。
...
| `mouse` | マウスクリック | `"LEFT"`, `"RIGHT"`, `"MIDDLE"`, `"LEFT_DOUBLE"` | 現在のカーソル位置でクリック |
| `wait` | ステップ間待機 | `"0.5"` | アプリのレスポンス待ち用（秒指定、小数可） |
| `dialog` | 確認ダイアログ | `"入力先をアクティブにして OK"` | メッセージを表示し、OK 押下まで次へ進まない |
| `repeat` | 繰り返し開始 | `"3"` | 直後から対応する `end_repeat`（「ここまで」）までの手順を `value` 回くりかえす（1〜9999、ネスト可） |
| `end_repeat` | 繰り返し終了（ここまで） | `""` | 直前の未閉じ `repeat` を閉じる。GUI では「──────── ここまで ────────」と表示 |

フラットな配列のまま開始／終了マーカーで囲む（ネストした `actions[]` は使わない）。保存時に対応関係と回数を検証する。
...
- **右側パネル**: マクロ詳細編集領域
  ...
  - **アクションリスト**:
    - 列: 順序 (#), 種別, 設定値
    - **複数選択**: Ctrl / Shift（Extended）
    - 操作: テキスト / キー / ショートカット / マウス / ウェイト / 確認 / 繰り返し（選択範囲を囲む／空ブロック挿入）、上下移動、削除
    - 繰り返し行はインデントとアクセントバーで示し、終了行は「ここまで」を明示表示する
  - **最下部**: [保存] [破棄] [テスト実行／中断] [パス＋引数をコピー]
  - 未保存のまま別マクロへ切り替えると保存／破棄／キャンセルを確認する
  - テスト実行中は「中断」で CancellationToken により停止可能
...
5. **`dialog` 処理**:
   - `value` をメッセージ本文として Win32 `MessageBox` を表示（空の場合は既定文）。
   - 他アプリの裏に回らないよう、最前面オーナー＋ `WS_EX_TOPMOST` の再適用で常に前面表示する。
   - ユーザーが OK を押すまでブロックし、押下後に次のアクションへ進む。
   - GUI テスト実行・CLI 実行の両方で表示する（手動確認ポイントとして利用）。
6. **手順間隔（アプリ設定）**:
   - `settings.json` の `action_delay_sec`（既定 `0.2`）を、各アクション実行後（最後を除く）に自動挿入する。
   - `0` にすると無効。個別の `wait` アクションと併用可能。
7. **`repeat` / `end_repeat` 処理**:
   - スタックでネストを解釈し、対応する終了マーカーまでを指定回数実行する。
   - マーカー自体はキー送信せず、空ボディはスキップする。
   - 周回のあいだにも手順間隔を入れる（最終周回の末尾には余分な間隔を入れない）。
...
2. **実行中の例外**:
   - キー送信中の予期せぬエラーはキャッチし、データフォルダの `error.log`（書けない場合は `%TEMP%\KeyAutomator-error.log`）にタイムスタンプ付きで記録。
