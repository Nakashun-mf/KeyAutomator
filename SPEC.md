...
- GUI の「起動パスをコピー」は、バッチやタスク スケジューラに貼る実行ファイルのフルパスをクリップボードへ入れる。
  パッケージ実行時は AppExecutionAlias（`%LocalAppData%\Microsoft\WindowsApps\KeyAutomator.exe`）を返す。設定フォルダ（config.json）とは別。
